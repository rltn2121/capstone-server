...
import com.capstone.mountain.infra.auth.KakaoProfile;
import com.capstone.mountain.infra.auth.NaverProfile;
import com.capstone.mountain.infra.auth.OAuthToken;
import com.capstone.mountain.infra.auth.PrincipalDetails;
import com.capstone.mountain.module.model.domain.User;
import com.capstone.mountain.module.model.dto.AccessTokenDto;
...
public class UserController{
    ...
    public ResponseEntity<Message> kakaoLogin(@RequestBody Map<String, String> req, HttpServletResponse response) throws AuthenticationException {
        ...
        return getJwt(response, username, password, nickname);
    ...
    public ResponseEntity<Message> naverLogin(@RequestBody Map<String, String> req, HttpServletResponse response) throws AuthenticationException {
        ...
        String password = "qwerty123";
        String nickname = kakaoProfile.getResponse().getName();

        // 가입자 혹은 비가입자 체크 해서 처리
        return getJwt(response, username, password, nickname);
    }

    private ResponseEntity<Message> getJwt(HttpServletResponse response, String username, String password, String nickname) {
        // 가입자 혹은 비가입자 체크 해서 처리
        User originUser = userService.chkUserExist(username);
        if (originUser.getUsername() == null) {
            System.out.println("기존 회원이 아니기에 자동 회원가입을 진행합니다");
